	...
	for(j = 0; j < ifd_count; j++){
		...
		switch(ifd_tag){
			...
			case 0x0111: // TIFF strip offset(s)
				...
				strip_count = byte_long(&buffer[4], endian);
				if(strip_count == 1){
...
{
	int x, y, row_offset, value, top_value, range;
...
void matrix_2d::forward(point_2d &p) const 
{
	...
	p.x = matrix_forward[0] * temp.x + matrix_forward[1] * temp.y + matrix_forward[2];
	p.y = matrix_forward[3] * temp.x + matrix_forward[4] * temp.y + matrix_forward[5];
}
void matrix_2d::reverse(point_2d &p) const
{
	point_2d temp(p);

	p.x = matrix_reverse[0] * temp.x + matrix_reverse[1] * temp.y + matrix_reverse[2];
	p.y = matrix_reverse[3] * temp.x + matrix_reverse[4] * temp.y + matrix_reverse[5];
}
