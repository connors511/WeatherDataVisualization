...
All radars will play after a single click and pause the animation if click while an animation is running. A double click will reset the radar to the first image and display the image for an inactive radar.\todo[inline,color=red]{Rewrite to suggestions and present time. Possibly write with other radar related considerations}
...
\hline
\textbf{License} & ``CC BY-SA''\footnote{Creative Commons Attribution-ShareAlike 2.0-license: Anyone can edit, open source (\url{https://creativecommons.org/licenses/by-sa/2.0/})} & ``NAVTEQ'' / ``TANA''\footnote{Navteq North America LLC / Tele Atlas North America: Owned by multiple organisations, data i copyrighted (\url{maps.google.co.uk/help/legalnotices_maps.html})} \\
\hline
\textbf{API} & Yes & Yes \\
\hline
\end{tabular}
