...
\label{sec:why_cpp_application}
The reason for using a C++ application because it is much faster to parse huge files in C++ than PHP or JavaScript.
...
\label{sec:why_qt}
It was decided to use Qt because of the support and community around it. It's very mature and most importantly, object oriented.
...
\todo[fancyline,color=green]{Adam @ sentence: ?}jQuery\footnote{JavaScript framework, see \cite{jquery} for the documentation} has been used as JavaScript framework, because of the very well documentation. jQuery is one of the most popular JavaScript library and is free open source software, so there is a lot of help to get on the web.\\
...
\subsection{Radar}
\label{sec:radar}
\subsubsection{Radar annotation}
...
The idea of a globally controlling `play' button sounded cool because it would enable one to view an animation of a greater area, but after a test implementation it was not working exactly as intended and therefore gave a weird feeling. This idea was dropped, but could be subject of future work.

The second idea of a local control was implemented and in the current version of the application all radars will play after a single click and pause the animation if clicked while an animation is running. A double click will reset the radar to the first image and display the image for an inactive radar.
...
\section{The data}
\label{sec:implementation_the_data}
