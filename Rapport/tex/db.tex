\section{Database}
\todo[inline]{Should database be split up into already existing sections e.g. analysis, keep like this or a third option?}
...
For safety reasons, the application should also contain user authentication in order to get certain administration privileges (such as file CUD\todo[fancyline,color=green]{Whats this? Footnote}). The database should therefore also handle data management for users.
...
Below is a diagram of the database has been implemented based on the analysis and design.\todo[fancyline,color=red]{reference figure instead of below}
...
These relations can be seen in the diagram. The relations and actions is stated by sql as\todo[fancyline,color=red]{reference to listing}
...
\end{lstlisting}\todo[fancyline,color=red]{add caption}
